...
import com.example.batch.model.JobExceptionStatusResponse;
...
public class BatchResource {
    ...
    public BatchResource(
    	    @ConfigProperty(name = "batch.chunk-size") String chunkSize,
    	    @ConfigProperty(name = "batch.job-name") String jobName,
            JobOperator jobOperator, BookRepository bookRepository) {
    ...
    public Response getStatus(@PathParam("executionId") long executionId) {
        try {
            ...
            var batchStatus = Objects.isNull(jobExecution.getExitStatus())
                    ? jobExecution.getBatchStatus().toString()
                    : jobExecution.getExitStatus();
            return Response.ok(new JobStatusResponse(executionId, jobExecution.getJobName(), batchStatus)).build();
        } catch (NoSuchJobExecutionException e) {
            return Response.status(Response.Status.NOT_FOUND)
                    .entity(new JobExceptionStatusResponse(executionId, "", "Unknown", e.getMessage()))
                    .build();
        }
    }
